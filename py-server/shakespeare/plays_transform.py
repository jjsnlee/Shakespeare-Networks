...
def generate_shakespeare_files(gen_imgs=False, gen_md=False, gen_lines=False, limit_plays=[], reset=False):
    ...
    from server.shakespeare.plays_n_graphs import get_plays_ctx, init_play_imgs
    import json
...
class PlayJSONContentEncoder(PlayEncoderBase):
    def default(self, obj):
        from shakespeare.plays_n_graphs import Play, Scene, Character, Line
        d = {}
...
class PlayJSONMetadataEncoder(PlayEncoderBase):
    def default(self, obj):
        from shakespeare.plays_n_graphs import Play, Scene

        if isinstance(obj, Play):
            d = self.from_keys(obj, ('title', 'year', 'type'))
            d['acts'] = self.get_acts_from_play(obj)
            d['characters'] = characters = list(obj.characters.keys())
            d['char_data'] = char_data = {}

            playG = obj.totalG
            cnxs = nx.degree(playG)
            for idx, c in enumerate(characters):
                nlines = playG.nodes[c]['nlines']
                ratio = 0
                if cnxs[c] > 0:
            ...
            for c in sceneG.nodes:
                nlines = sceneG.nodes[c]['nlines']
                total_lines += nlines
                ratio = 0
                degrees = cnxs[c]
                if cnxs[c] > 0:
