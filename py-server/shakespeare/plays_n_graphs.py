...
# import matplotlib
# matplotlib.use('Agg')
from shakespeare import plays_cfg
...
class ChekhovPlayCtx(RootPlayCtx):
    ...
    def _load_play(self, play, play_alias):
        ...
        play_dialogue_ptn = r'<A NAME=(\d+).(\d+).(\d+)><b>(.+?)\.</b>(.+?)</a>'
...
class ShakespearePlayCtx(RootPlayCtx):
    ...
    def _load_play(self, play, play_alias):
        ...
        acts_ptn = r'Act (\d+), Scene (\d+): <a href="([^\"]+)">([^>]+)</a>'
        prologue_ptn = r'Act (\d+), Prologue: <a href="([^\"]+)">([^>]+)</a>'
        # For Taming of the Shrew
        prologue_ptn2 = r'Induction, Scene (\d+): <a href="([^\"]+)">([^>]+)</a>'
        # For Henry IV, Pt 2
        prologue_ptn3 = r'Induction: <a href="([^\"]+)">([^>]+)</a>'
        ...
        play_dialogue_ptn = r'<A NAME=speech\d+><b>([^<]+?):{0,1}</b></a>(.+?(?:</body>|</blockquote>))'
...
def _init_graphs(play):
    ...
    for sc in play.scenes:
        ...
        for speaker, lines in sc.dialogues:
            ...
            G.nodes[speaker]['nlines'] += len(lines)
            if prev_speaker:
...
class Play(object):
        ...
        if not self._totalG:
            ...
            for sc in self.scenes:
                ...
                for speaker in G_.nodes:
                    ...
                    if speaker in origTG.nodes:
                        totalG.nodes[speaker]['nlines'] = \
                            G_.nodes[speaker]['nlines'] + origTG.nodes[speaker]['nlines']
    ...
    def scenes(self):
        if self._scenes is None:
            sc_keys = list(self.scenes_idx.keys())
...
class Scene(object):
    ...
    def dialogue_matrix(self):
        if self._mat is not None:
            seq_lines = []
            all_chars = list(self.play.characters.keys())
            for character, lines in self.dialogues:
...
def main(title_to_run=''):
    ...
    # print pformat(plays)
    for play_graph in list(ctx.play_details.values()):
