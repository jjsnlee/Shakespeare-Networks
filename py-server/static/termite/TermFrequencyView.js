...
TermFrequencyView.prototype.highlight = function( term, topic ) {
	...
	// highlight topic
		...
		// highlight labels
		var stackedData = this.prepareStackedBars();
		var selectedTopics = this.parentModel.get("selectedTopics");
		var colors = this.parentModel.get("colorList").slice();
		
		// decide how to "highlight" bars
		if( selectedTopics[topic] !== null){
			// previously selected topic
			this.prevHighlightColor = selectedTopics[topic];
			colors[colors.indexOf(selectedTopics[topic])] = HIGHLIGHT;
			this.gLayer = this.overlayLayer.selectAll( "g" )
